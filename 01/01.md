...
* For example, to find all rows in the salaries file containing the text `'EMANUEL'`:
```bash
$ grep EMANUEL salaries.csv
"BUFORD,  EMANUEL L",TRAFFIC CONTROL AIDE-HOURLY,OEMC,P,Hourly,20,,$19.86
"EMANUEL,  RAHM ",MAYOR,MAYOR'S OFFICE,F,Salary,,$216210.00,
"LEON,  EMANUEL E",HOSPITALITY WORKER,FAMILY & SUPPORT,P,Hourly,20,,$13.00
"MUHAMMAD,  EMANUEL ",POLICE OFFICER,POLICE,F,Salary,,$93354.00,
"ROBINSON,  EMANUEL D",POOL MOTOR TRUCK DRIVER,STREETS & SAN,F,Hourly,40,,$35.60
```

### `grep` regular expressions
* Regular expressions are special strings that match complex patterns
* They make `grep` very, very powerful
* For example, this regex only matches lines that *start* with `"EMANUEL`:
...
```bash
$ grep EMANUEL salaries.csv > salaries_emanuel.csv
```
...
* Then to count the number of police officers we *pipe* to `wc`:
```bash
...
* But command line will remain relevant in two ways:
    ...
    * Tools like `grep`, `wc`, `curl`, etc. will remain important for a fast and efficient first pass in programming and data analysis
